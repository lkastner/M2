...
append'-*(MutableList, List)*- := (M, L) -> scan(L, x -> M##M = x)
...
ccRefinement Matrix := M -> (
     ...
     -- Selecting those cones that are 'n' dimensional and do not contain any of the others
     nConesfd := select(nCones, C -> dim C == n);
     -- this can get even slower
     nConesfd = inclMinCones nConesfd;
     refCones := new MutableList;
     -- TODO: implement a heap version?
     while nConesfd != {} do (
	  newCones := new MutableList;
	  ...
	  append'_refCones flatten apply(nConesfd, C1 -> (
			 toBeAdded := flatten apply(nCones,
			     C2 -> (
				 -- make sure this isn't calculated twice
				 C := intersection(C2,C1);
				 -- check containment first maybe?
				 if dim C == n and (not contains(C2,C1)) then C else {}));
			 if toBeAdded == {} then C1
			 else ( append'_newCones toBeAdded; {})));
	  ...
	  nConesfd = unique toList newCones);
